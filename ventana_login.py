...
def ventana_inicio():
    ...
    ventana_principal.geometry("500x350")#DIMENSIONES DE LA VENTANA
    ventana_principal.title("Login con tkinter")#TITULO DE LA VENTANA
    Label(text="Escoja su opción", bg="LightBlue", width="300", height="2", font=("Calibri", 13)).pack()#ETIQUETA CON TEXTO
    Label(text="").pack()
    ...
    Label(text="").pack()
    Button(text="Subir archivo", height="2", width="30", bg=pestas_color, command=recuperar).pack() #BOTÓN "subri archivo".
    Label(text="").pack()
    Button(text="Generar contraseña", height="2", width="30", bg=pestas_color, command=password).pack()
...
def password():
	...
	global ventana_pass
	ventana_pass = Toplevel(ventana_principal)
	ventana_pass.title("Contraseña generada")
	ventana_pass.geometry("300x70")
	Label(ventana_pass, text="Su contraseña es: " + p).pack()
	Button(ventana_pass, text="OK", command=borrar_ventana).pack() 



def recuperar():
	global nombrearch
	nombrearch=fd.askopenfilename(initialdir = "/",title = "Seleccione archivo",filetypes = (("csv files","*.csv"),("todos los archivos","*.*")))
	if nombrearch!='':
...
def registro():
    ...
    Label(ventana_registro, text="Introduzca datos", bg="LightBlue").pack()
